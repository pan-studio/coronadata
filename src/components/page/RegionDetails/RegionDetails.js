...
import rawRegion from '../../../data/region/region';
import rawRegionLogs from '../../../data/region/regionLogs';

const RegionSummary = () => {
  const { loading, data: region } = useFakeFetch(rawRegion);
  const { name, email, createdAt } = region;
          ...
          <Fragment>
            ...
            <CardBody className="border-top">
              <Media>
                ...
                <Media body>
                  <p className="mb-0">Region was created</p>
...
const RegionDetailRow = ({ title, isLastItem, children }) => (
  <Row>
    <Col xs={5} sm={4}>
      <p
...
RegionDetailRow.propTypes = {
  title: PropTypes.string.isRequired,
  children: PropTypes.node.isRequired,
  isLastItem: PropTypes.bool
};

RegionDetailRow.defaultProps = { last: false };

const RegionDetail = () => {
  const { loading, data: region } = useFakeFetch(rawRegion);
  const { id, email, createdAt, description, vat_no, email_to, address, cell, invoice_prefix } = region;
          ...
          <Loader />
            ...
            <Row>
              <Col lg className="col-xxl-5">
                ...
                <RegionDetailRow title="ID">{id}</RegionDetailRow>
                <RegionDetailRow title="Created">{createdAt}</RegionDetailRow>
                <RegionDetailRow title="Email">
                  <a href={`mailto:${email}`}>{email}</a>
                </RegionDetailRow>
                <RegionDetailRow title="Description">
                  {description ? description : <p className="font-italic text-400 mb-1">No Description</p>}
                </RegionDetailRow>
                <RegionDetailRow title="VAT number" isLastItem>
              ...
              <Col lg className="col-xxl-5 mt-4 mt-lg-0 offset-xxl-1">
                ...
                <RegionDetailRow title="Send email to">
                  <a href={`mailto:${email_to}`}>{email_to}</a>
                </RegionDetailRow>
                <RegionDetailRow title="Address">
                  <p className="mb-1" dangerouslySetInnerHTML={createMarkup(address)} />
                </RegionDetailRow>
                <RegionDetailRow title="Phone number">
                  <a href={`tel:${cell}`}>{cell}</a>
                </RegionDetailRow>
                <RegionDetailRow title="Invoice prefix" isLastItem>
...
const RegionLog = ({ status, link, time }) => {
...
const RegionLogs = () => {
  const { loading, data: regionLogs } = useFakeFetch(rawRegionLogs);
      ...
      <CardBody className="border-top p-0">
        {loading ? (
          <Loader />
            ...
            isIterableArray(regionLogs) && regionLogs.map((log, index) => <RegionLog {...log} key={index} />)
          )}
      </CardBody>
